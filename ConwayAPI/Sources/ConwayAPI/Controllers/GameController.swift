    ...
    func simulate(req: Request) async throws -> GameSimulationResponse {
        ...
        for generation in 1...request.generations {
            ...
            if convergence != .continuing {
                let period: Int? = switch convergence {
                case let .cyclical(p):
                    p
                default:
                    nil
...
enum GridValidationError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case let .emptyRow(row):
            "Row \(row) cannot be empty"
        case let .inconsistentWidth(row, expected, actual):
            "Row \(row) has inconsistent width: expected \(expected), got \(actual)"
        case let .gridTooLarge(width, height, maxW, maxH):
            "Grid too large: \(width)x\(height). Max allowed is \(maxW)x\(maxH)"
        }
    }
}
